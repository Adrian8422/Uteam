  ...
  const handleFormSubmit = async (characterData: any, id?: number) => {
    try {
      if (id) {
        ...
        const updatedCharacter = await marvelApi.updateCharacter(id, characterData);
        if (updatedCharacter) {
      ...
      {showForm ? (
        ...
        <CharacterDetail 
          ...
          onDelete={async () => {
            if (window.confirm('¿Estás seguro que deseas eliminar este personaje?')) {
              try {
                const success = await marvelApi.deleteCharacter(selectedCharacter.id);
                if (success) {
        ...
        <Grid>
          {characters.map(character => (
            <Card key={character.id} onClick={() => handleCharacterClick(character)}>
              <CardImage 
              ...
              <CardContent>
                ...
                <Button onClick={(e) => handleEditClick(character, e)}>Editar</Button>
                <Button onClick={(e) => handleDeleteClick(character.id, e)}>Eliminar</Button>
              </CardContent>
            </Card>
          ))}
        </Grid>
